...
impl Renderer {
    ...
    pub fn new() -> Self {
        let mut vao = VertexArray::cube();
        vao.make_instanced(Self::MAX_COUNT);
    ...
    pub fn draw(&mut self, scene: &mut Scene, window: &Window) {
        ...
        if let Some(buffer) = self.vao.get_instanced_buffer() {
            buffer.bind();
            ...
            self.shader
                .set_mat4("view", &scene.get_camera().get_matrix());
